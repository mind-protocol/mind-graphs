...
async function refresh() {
  ...
  try {
    const [summaryResponse, shadowResponse] = await Promise.all([
      fetch("/api/l1/subentities/summary", { cache: "no-store" }),
      fetch("/api/l1/subentities/shadow/", { cache: "no-store" })
    ]);
    ...
    connection.textContent = data.projection.status === "current" ? `Révision ${data.revision} · synchronisée` : `Révision ${data.revision} · projection à réparer`;
