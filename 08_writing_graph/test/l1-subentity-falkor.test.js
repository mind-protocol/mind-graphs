...
const state = {
  ...
  relations: [{ id: "narrative-describes-part", source: "narrative-a", type: "DESCRIBES_SUBENTITY", target: "part-a" }],
...
test("runtime projection maps every state family to the L1 ontology", () => {
  ...
  assert.deepEqual(projection.nodes.map(node => node.props.nodeType).sort(), ["lifecycle_event", "memory", "subentity", "subentity_narrative"]);
