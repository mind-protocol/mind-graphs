...
import { applyFalkorIntegratedL1Tick, summarizeSubentityRuntime } from "./l1-integrated-runtime.js";

export function createL1SubentityRouter({ getGraph, readState = readFalkorSubentityState, applyTick = applyFalkorSubentityLifecycleTick, applyIntegratedTick = applyFalkorIntegratedL1Tick, repairProjection = repairFalkorSubentityProjection }) {
  if (typeof getGraph !== "function") throw new Error("createL1SubentityRouter requires getGraph.");
  const router = express.Router();

  router.get("/state", async (_req, res) => {
    try {
      const current = await readState(await getGraph());
  ...
  router.post("/ticks", async (req, res) => {
    try {
      const result = await applyTick({ graph: await getGraph(), input: req.body });
  ...
  router.post("/integrated-ticks", async (req, res) => {
    try {
      const result = await applyIntegratedTick({ graph: await getGraph(), input: req.body });
      const status = result.report.status === "already_processed" ? 200 : result.projectionStatus === "repair_required" ? 202 : 201;
  ...
  router.post("/projection/repair", async (_req, res) => {
    try {
      res.json(await repairProjection(await getGraph()));
    } catch (error) {
      res.status(503).json({ error: error.message });
    }
  });
