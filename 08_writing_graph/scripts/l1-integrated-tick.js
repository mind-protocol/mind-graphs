...
import { runIntegratedL1Tick, applyFalkorIntegratedL1Tick } from "../src/l1-integrated-runtime.js";
...
if (backend === "file") {
  ...
  result = runIntegratedL1Tick(previous, input);
  ...
  try {
    if (dryRun) {
      ...
      result = runIntegratedL1Tick(current.state, input);
      result.persisted = false;
      result.projectionStatus = current.revision === current.projectionRevision ? "current" : "repair_required";
    } else {
      result = await applyFalkorIntegratedL1Tick({ graph, input });
    }
  } finally {
