...
Les **types sémantiques** (`semanticType`) constituent un **champ libre d'étiquettes et de spécialisations métier** (`protocol`, `axiom`, `mechanism`, `system_state`, `open_question`, `source_document`, etc.). Dans la couche physique L4, chaque type sémantique se projette sur l'un de ces **5 rôles physiques fondamentaux** (voir `data/l4-ontology-mapping.json`).
