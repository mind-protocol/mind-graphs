...
async function validateGraph(graphConfig) {
  ...
  const nodeTypeIds = new Set((ontology.nodeTypes || []).map(type => type.id || type.type));
  const semanticTypeIds = new Set((ontology.semanticTypes || ontology.nodeTypes || []).map(type => type.id || type.type));
  ...
  const roleIds = new Set((ontology.roleAxis?.roles || ontology.nodeTypes || []).map(role => role.id || role.type));
  ...
  for (const type of (ontology.semanticTypes || ontology.nodeTypes || [])) {
    for (const field of ["id", "label", "family", "epistemicStatus", "description"]) if (!type[field] && !type.type) errors.push(`node type ${type.id || type.type || "?"} misses ${field}`);
    if (epistemicStatusIds.size > 0 && type.epistemicStatus && !epistemicStatusIds.has(type.epistemicStatus)) errors.push(`node type ${type.id} has unknown epistemic status ${type.epistemicStatus}`);
  }
  for (const type of (ontology.relationTypes || ontology.linkTypes || [])) {
    if (familyIds.size > 0 && type.family && !familyIds.has(type.family)) errors.push(`relation ${type.id || type.type} has unknown family ${type.family}`);
    if (type.status && !["active", "reserved"].includes(type.status)) errors.push(`relation ${type.id || type.type} has invalid status`);
  }
